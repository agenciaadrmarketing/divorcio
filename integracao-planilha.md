...
function notificar(nome, email, whats, d, agora) {
  ...
  '<div style="font:14px Arial,sans-serif;color:#ffffff;padding-top:8px;line-height:1.5;">Um visitante preencheu o popup do site sobre revisão de contrato.</div>' +
  '</td></tr>' +
  ...
  var assunto = 'NOVO LEAD (Juros Abusivos) - Vieira & Marques - ' + (nome || 'sem nome');
  var res = [];
...
function testar() {
  Logger.log(salvarLead({
    ...
    mensagem: 'Oi, quero uma análise do meu contrato.',
    origem_form: 'popup', pagina: 'https://advogado-golpedopix.vieiraemarquesadvogados.com/'
  }).getContent());
}
```
